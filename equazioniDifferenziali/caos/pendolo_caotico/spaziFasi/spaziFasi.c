...
/*
  Questo programma integra le equazioni del moto del pendolo caotico con moduli della forzante diversi per studiare le traiettorie nello spazio delle fasi. Si utilizza il metodo di runge kutta al quarto ordine.
 */
...
int main()
{
  double x0 = M_PI/2., v0 = 0, dt = 0.001, totalTime = 250, gamma = 0.5, omega = 1, omBarra = 2./3., f0[NINT] ={0.9, 1.07, 1.15, 1.35, 1.47, 1.50};
  ...
  for (i = 1; i < numberOfSteps; i++) {
    for (j = 0; j < NINT; j++) {
      ...
      // buttiamo i primi 50 secondi
      if (i * dt > 50) printf ("%lf %lf ", xandV[j].x, xandV[j].v);
    }
    if (i * dt > 50) printf ("\n");
  }
